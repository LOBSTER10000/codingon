document.addEventListener('DOMContentLoaded', function(){
    ...
    form.addEventListener('submit', function(e){
        ...
        .then((res)=>{
            ...
            if(res.data.success){
                alert('성공하셨습니다');
               result1.textContent = JSON.stringify(res.data.result.writer).replace(/"'/g,'');
               result2.textContent = JSON.stringify(res.data.result.header).replace(/"'/g,'');
               result3.textContent = JSON.stringify(res.data.result.content).replace(/"'/g,'');
            }
            else{
                alert('실패했습니다');
            }
